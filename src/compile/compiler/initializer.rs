...
pub fn initialize<'a>(
    ...
    let lib_paths = &compile_options().options().typst_lib.paths;
...
pub fn init_options_toml(config_path: &Path) -> Result<()> {
    let new_options = Options::load(config_path)?;
    compile_options().options = Some(new_options);
    Ok(())
}
